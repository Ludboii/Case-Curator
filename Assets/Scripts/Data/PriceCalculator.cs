public static class PriceCalculator
{
    ...
    private static float GetUniquePatternMultiplier(InventoryItem item)
    {
        if (item.skin.uniquePatterns == null)
            return 1f;

        foreach (SpecialPattern specialPattern in item.skin.uniquePatterns)
        {
    ...
    private static float GetTierMultiplier(InventoryItem item)
    {
        ...
        foreach (PatternTierGroup group in item.skin.patternTierGroups)
        {
            if (group.tier == item.patternTier)
    ...
    private static float GetFadeMultiplier(InventoryItem item)
    {
        ...
        float fadePercent = FadeUtility.GetFadePercent(
            item.skin,
            item.patternId);
    ...
    {
        // Low-float multipliers from the Case Curator balance workbook.
        if (floatValue < 0.00001f) return 4.50f;
        if (floatValue < 0.0001f) return 2.80f;
        if (floatValue < 0.001f) return 1.95f;
        if (floatValue < 0.01f) return 1.30f;

        // High-float multipliers from the Case Curator balance workbook.
        if (floatValue > 0.999f) return 1.75f;
        if (floatValue > 0.99f) return 1.30f;
        if (floatValue > 0.98f) return 1.15f;
        if (floatValue > 0.95f) return 1.05f;
